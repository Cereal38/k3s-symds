...
echo "\=== Remove existing SymmetricDs and Flyway things in Oracle ===\n"
kubectl exec deploy/symds-oracle -- /opt/symmetric-ds/bin/symadmin --engine oracle-000 uninstall
kubectl exec -i oracle-db -- sqlplus -S "ora/ora@//localhost:1521/FREEPDB1" <<< "drop table \"flyway_schema_history\" purge;"

echo "\n=== Remove location service symds deployment ===\n"
kubectl delete -f k8s/symds/symds-location-deployment.yaml
kubectl wait --for=delete pod -l app=symds-location --timeout=120s || true

echo "\n=== Remove Oracle symds deployment ===\n"
kubectl delete -f k8s/symds/symds-oracle-deployment.yaml
kubectl wait --for=delete pod -l app=symds-oracle --timeout=120s || true

echo "\n=== Delete location postgres db ===\n"
kubectl delete --ignore-not-found \
  -f k8s/db/location-db.yaml \
  -f k8s/configmap/location-sql-script-configmap.yaml \
  -f k8s/job/location-setup-user-conf-job.yaml

kubectl wait --for=delete pvc -l cnpg.io/cluster=location-pg --timeout=240s || true
...
echo "\n=== Wait for triggers to be created and migration to be finished ===\n"
kubectl rollout status deploy/symds-location --timeout=300s

echo "\n=== Try to insert Grenoble3 in DB ===\n"
...
kubectl wait --for=condition=complete --timeout=120s job/location-setup-user-conf

echo "\n=== Try to insert Grenoble5 in DB ===\n"
